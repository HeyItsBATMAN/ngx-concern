...
@Component({
  ...
  styles: `
    ...
    @keyframes dialog-content-fade-in {
      from {
        transform: scale(0.9);
        opacity: 0;
      }
    ...
    :host[data-state='closed'] div.dialog-content {
      display: none;
    }

    :host[data-state='prepare'],
    :host[data-state*='open'] div.dialog-content {
      display: block;
      animation: dialog-content-fade-in 0.3s ease;
    }
    ...
    :host[data-state*='clos'] div.dialog-content {
      transform: scale(0.9);
      opacity: 0;
    }
  `,
...
export class DialogComponent extends BaseComponent implements OnInit {
  ...
  #defaultDialogStyle = inject(CONCERN_DIALOG_STYLE);
